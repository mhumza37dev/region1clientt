...
import { div, divBody, divColumns, divHeader } from "reactstrap";
...
//.map function use karke array banai han color codes ke leye let colorCode = data.map(()  => return(math + '#')) == save in setColor(colorCode)
//.map data.map return candidate names this will be used in label == save in usestate
// data.map return candidate voteCount == save in usestate

//candidate names list jo fetch hui han
...
const Charts = () => {
  ...
  const [color, setColor] = useState(["#000", "#fff"]);
  const [candidateName, setCandidateName] = useState();
  const [count, setCount] = useState([10, 52]);
  ...
  useEffect(() => {
    ...
    fetch("https://region1server.herokuapp.com/candidates")
      ...
      .then((res) => {
        ...
        let count = [];
        for (let x in res) {
          // console.log(res[x].Fullname)
          name.push(res[x].Fullname);
          count.push(parseInt(myDecipher(res[x].VoteCount)));
          ...
          {election !== undefined && election.status !== "started" ? (
            <div>
              ...
              <div>
                ...
                >
                  {/* <Paper elevation={3} style={{ margin: "", padding: "2%" }}> */}
